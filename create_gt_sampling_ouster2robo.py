...
for sensor in sensors:
    database_save_path = Path(data_path) / ('gt_database_%s2robo_mod126' % (sensor) if split == 'train' else 'gt_database_%s_2modules' % ( split))
    db_info_save_path = Path(data_path) / ('gt_database_info_%s2robo_mod126.pkl' % (sensor) if split == 'train' else 'pseudo_gt_database_info_%s_2modules.pkl' % (sensor, split))
