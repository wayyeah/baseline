...
database_save_path = Path(data_path) / ('pseudo_gt_database_%s2%s_ours' % (source_sensor,target_sensor) if split == 'train' else 'gt_database_%s2%s_%s_ours' % (source_sensor,target_sensor, split))
db_info_save_path = Path(data_path) / ('pseudo_gt_database_info_%s2%s_ours.pkl' % (source_sensor,target_sensor) if split == 'train' else 'pseudo_gt_database_info_%s2%s_%s_ours.pkl' % (source_sensor,target_sensor, split))
print("save Path:",database_save_path)
print('len info when create gt_database: ', len(result))
database_save_path.mkdir(parents=True, exist_ok=True)
all_db_infos = {}
print('generation gt_database for sensor: %s' % target_sensor)
for i in range(len(result)):
    ...
    #DISTANCE_BEAM_DOWNSAMPLE
    if((target_sensor=="ouster" and source_sensor=="hesai") or (target_sensor=="robosense" and source_sensor=="hesai")):
        lines_kept = [18,21,24,27,30,32,35,38, \
                              41,44,47,49,52,55,58,61, \
                              64,66,69,72,75,78,80,83, \
                              86,89,92,95,97,100,103,106]
        mask_line= np.isin(points[:, 4], lines_kept)
        distance = np.sqrt(np.sum(points[:, :2]**2, axis=1))
