...
class XMechanismUnmanned(DatasetTemplate):
    ...
    def get_lidar(self, idx, sensor, num_features=4):
        ...
        # exit()
        assert self.dataset_cfg.get('BOX_COX_NORM_INTENSITY', False), 'BOX_COX_NORM_INTENSITY is not True'

        if sensor == 'ouster' and self.dataset_cfg.get('DOWN_SAMPLE_TO_HESAI', False):
            pc_ring = lidar.pc_data['ring']
        
        elif self.dataset_cfg.get('BOX_COX_NORM_INTENSITY', False):
        ...
        assert not self.dataset_cfg.get('GRADUALLY_DOWNSAMPLE', False), 'currently not support.'
        if self.dataset_cfg.get('GRADUALLY_DOWNSAMPLE', False):
            downsample_every_n_epoch =  self.dataset_cfg.get('DOWNSAMPLE_EVERY_N_EPOCH', 16)
            downsampe_rate = 0.25 * (int(self.cur_epoch / downsample_every_n_epoch))

            if np.random.rand() < downsampe_rate:
                lidar_file_list = os.listdir(os.path.join(self.root_path, 'downsample','seq'+seq, sensor+'_distance_beam_downsample'))
                lidar_file = os.path.join(self.root_path, 'downsample','seq'+seq, sensor+'_distance_beam_downsample', lidar_file_list[int(frame)])
        
        lidar = np.stack([pc_x, pc_y, pc_z, pc_i], axis=1)
        # print(lidar.shape)
        
        if sensor == 'ouster' and self.dataset_cfg.get('DOWN_SAMPLE_TO_HESAI', False):
            # lidar = lidar[pc_ring % 4 == 0]
            ...
            # print(lidar.shape)
            # exit()
            # self.downsample_ouster_to_hesai(lidar[:, :3], pc_ring)
