...
class PointFeatureEncoder(object):
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        for x in self.used_feature_list:
            ...
            # normalize intensity add from ssda3d
            # use log to narrow the spatial feature
            # assert self.point_encoding_config.get('normalize_intensity', None), "normalize_intensity is not set"
            if x == 'intensity' and self.point_encoding_config.get('normalize_intensity', None):
                # points[:, idx:idx+1] = points[:, idx:idx+1] / max(points[:, idx:idx+1])
                points[:, idx:idx+1] = np.log(points[:, idx:idx+1] + 1) - np.log(np.min(points[:, idx:idx+1]) + 1) / (np.log(np.max(points[:, idx:idx+1]) + 1) - np.log(np.min(points[:, idx:idx+1]) + 1))
